            ...
            <table class="table table-hover table-striped">
                ...
                @foreach($offers as $offer)
                    <tr>
                        <td> {{$offer->title}} </td>
                        <td> {{$offer->options}} </td>
                        <td> <img src="{{asset("storage/".$offer->image)}}" alt="No Image Found" width="100"> </td>
                        <td> <input {{$offer->published?"checked":""}} type="checkbox" disabled> </td>
                        <td>
                            <div class="btn-group">
                                ...
                                @csrf
                                @method("DELETE")
                                </form>{{--<button href={{route("offers.destroy")}} type="button" class="btn btn-danger">Delete</buttonhref>--}}
                            </div>
                        </td>
                    </tr>
                @endforeach
